            ...
            {alerts.flatMap((alert) => {
              ...
              rows.push(
                <motion.tr
                  ...
                  onMouseEnter={(e) => { if (!isExpanded) (e.currentTarget as HTMLElement).style.background = C.hover }}
              ...
              if (isExpanded) {
                rows.push(
                  ...
                  >
                    <td colSpan={8} style={{ padding: '16px 24px', borderBottom: `1px solid ${C.border}` }}>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
                        ...
                        <div>
                          ...
                          {[
                            { label: 'Isolation Forest', score: alert.model_scores.isolation_forest },
                            { label: 'LSTM Autoencoder', score: alert.model_scores.lstm },
                            { label: 'XGBoost', score: alert.model_scores.xgboost },
                          ].map(({ label, score }) => (
                            <div key={label} style={{ marginBottom: 10 }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                                <span style={{ fontSize: 11, color: C.textMuted }}>{label}</span>
                                <span style={{ fontSize: 11, fontWeight: 700, color: score > 0.6 ? C.critical : score > 0.4 ? C.medium : C.low }}>
                                  {(score * 100).toFixed(0)}%
                                </span>
                              </div>
                              <div style={{ height: 4, background: '#30363D', borderRadius: 2, overflow: 'hidden' }}>
                                <div style={{ width: `${score * 100}%`, height: '100%', background: score > 0.6 ? C.critical : score > 0.4 ? C.medium : C.low }} />
                              </div>
                            </div>
                          ))}
                          <div style={{ marginTop: 16, display: 'flex', gap: 8 }}>
                            <button
                              onClick={(e) => { e.stopPropagation(); onSelectAlert?.(alert.id) }}
                              style={{
